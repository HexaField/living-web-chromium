...
//
// Standalone Graph Synchronisation Protocol engine (Spec 05).
//
// This header-only engine composes the Chromium-independent cores — the
// GraphDiff identity + sync-space derivation of graph_diff.{h,cc}, the Oxigraph
// rdfc-1.0 canonicaliser (Spec 02), the DID-document projection (Spec 03), and
// the GovernanceEngine (Spec 04) — into the full Spec 05 sync engine that the
// standalone harness exercises. It is the Chromium-independent mirror of the
// browser-process backend (content/browser/graph_sync/sync_backend.{h,cc}); the
// two build worlds share graph_diff.{h,cc} so the bytes a diff is
// content-addressed and signed over are byte-identical between them.
//
// What it implements, normatively:
//   * GraphDiff construction (§5.2.2): revision/commitId pre-images (pinned by
//     SPEC_COMPLIANCE amendment 05/§5.2.2.1) canonicalised with rdfc-1.0 over the
//     triples-with-reifiers, hashed with SHA-256, and signed by the committer's
//     key over the UTF-8 lowercase-hex commitId.
...
//     the GovernanceEngine (which itself honours §9.4 enforcement-mode
//     awareness), per-reifier signature verification, dependency validation
//     (§5.2.1), and timestamp plausibility (§14.5).
...
#ifndef LIVING_WEB_SYNC_PROVIDER_H_
#define LIVING_WEB_SYNC_PROVIDER_H_
...
// exactly those `BuildTripleWithReifierNquads` serialises, so the wire bytes a
// diff is canonicalised over reproduce on any peer.
struct DiffTriple {
  Triple triple;
...
// §5.3 CapabilityProof (the standalone engine resolves the chain from the target
// graph's local store; VerifiablePresentations are a browser/VC concern and are
// not modelled here).
...
struct CommitOptions {
  ...
  std::string timestamp;  // empty → graph_detail::NowRfc3339()
};
...
class SyncEngine {
 public:
  SyncEngine(DIDKeyProvider* identity, GovernanceEngine* governance)
  ...
  bool CommitDiff(Graph* W,
    ...
    diff.timestamp =
        opts.timestamp.empty() ? graph_detail::NowRfc3339() : opts.timestamp;
    ...
    diff.dependencies = SortDependencies(opts.dependencies);

    const std::string method =
        author->did + "#" +
        *did_key::Ed25519PublicKeyMultibase(author->public_key);
    ...
    diff.revision = ToLowerHex(crypto::SHA256HashString(BuildRevisionPreimage(
    ...
    const std::string leaf =
        (diff.capability_proof && !diff.capability_proof->chain.empty())
            ...
            : std::string();
    diff.commit_id = ToLowerHex(crypto::SHA256HashString(BuildCommitIdPreimage(
        diff.revision, diff.author, diff.timestamp, leaf)));

    const std::string msg = BuildSignatureMessage(diff.commit_id);
    auto sig = identity_->SignRaw(
        author->id, std::vector<uint8_t>(msg.begin(), msg.end()));
    if (!sig) {
      last_error_ = "InvalidStateError";  // locked/unknown credential
      return false;
    }
    diff.signature = did_key::MultibaseEncode(*sig);
  ...
  SyncValidationResult ValidateDiff(Graph* W, const GraphDiff& diff) {
    ...
    const std::string revision = ToLowerHex(crypto::SHA256HashString(
        BuildRevisionPreimage(diff.graph_did, canon_add, canon_rem,
                              SortDependencies(diff.dependencies))));
    ...
    const std::string leaf =
        (diff.capability_proof && !diff.capability_proof->chain.empty())
            ? diff.capability_proof->chain.front()
            : std::string();
    const std::string commit_id = ToLowerHex(crypto::SHA256HashString(
        BuildCommitIdPreimage(revision, diff.author, diff.timestamp, leaf)));
    ...
    // GovernanceEngine collects constraints (§6.2), walks the chain to
  ...
  // §7.1). Keys off the constraint's presence, not enforcement_mode (§7.2).
  bool IsRestricted(Graph* W) {
    std::optional<std::string> wdid = W ? W->did() : std::nullopt;
    if (!wdid)
      return false;
  ...
  std::optional<std::string> DeriveSpace(
    ...
    std::string namespace_id =
        group_detail::FirstIriOf(W, *wdid, kContextParticipatesIn)
            .value_or(*wdid);
    std::string input = BuildSpaceDerivationInput(
        topology, IsRestricted(W), namespace_id, *wdid, custom_name);
    return std::string(kSpaceScheme) +
           ToLowerHex(crypto::SHA256HashString(input));
  }
 ...
 private:
  ...
  // SHA-256(BuildSignaturePreimage(triple, timestamp, graphDid)) with the
  // committer's key. graphDid is the stable graph identifier a receiver
  ...
  bool AuthorReifiers(const std::string& graph_did,
    ...
    for (const Triple& t : triples) {
      std::string payload = crypto::SHA256HashString(
          BuildSignaturePreimage(t, timestamp, graph_did));
      auto sig = identity_->SignRaw(
  ...
  static bool CanonicalizeDiffTriples(const std::vector<DiffTriple>& triples,
    ...
    return OxigraphStore::Canonicalize(block, CanonHash::kSha256, out, err);
  }
  ...
  std::vector<std::vector<uint8_t>> ResolveAuthorKeys(Graph* W,
                                                      const std::string& author) {
    std::vector<std::vector<uint8_t>> keys;
    if (did_graph::IsDidGraph(author)) {
      DidDocument doc;
      if (group_detail::ProjectDidDocument(W, author, &doc) && !doc.deactivated) {
        for (const std::string& md : doc.capability_delegation) {
          const VerificationMethod* vm = doc.FindMethod(md);
          if (!vm)
            continue;
          if (auto pk = DecodePublicKeyMultibase(vm->public_key_multibase))
            keys.push_back(std::move(*pk));
        }
      }
    } else if (auto pk = ParseAnyDidEd25519(author)) {
      keys.push_back(std::move(*pk));
    }
    return keys;
  }
  ...
  bool VerifyBundleSignature(Graph* W,
                             const GraphDiff& diff,
                             const std::string& commit_id) {
    std::string msg = BuildSignatureMessage(commit_id);
    auto sig = did_key::MultibaseDecode(diff.signature);
    if (!sig || sig->size() != 64)
      return false;
  ...
  bool VerifyReifier(Graph* W,
    ...
    if (dt.author != diff_author)
      return false;
    std::string payload = crypto::SHA256HashString(
        BuildSignaturePreimage(dt.triple, dt.timestamp, graph_did));
    auto sig = did_key::MultibaseDecode(dt.signature);
    if (!sig || sig->size() != 64)
      return false;
...
#endif  // LIVING_WEB_SYNC_PROVIDER_H_
