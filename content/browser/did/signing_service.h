...
class SigningService : public graph::mojom::DIDCredentialService {
 public:
  ...
  void SignCapability(const std::string& credential_id,
                      const std::string& zcap_json,
                      SignCapabilityCallback callback) override;
  void SignRaw(const std::string& credential_id,
               const std::vector<uint8_t>& payload,
               SignRawCallback callback) override;
