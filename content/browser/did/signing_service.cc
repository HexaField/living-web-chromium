...
void SigningService::Sign(const std::string& credential_id,
  ...
  proof->method = result->proof_method;
...
void SigningService::SignCapability(const std::string& credential_id,
                                    const std::string& zcap_json,
                                    SignCapabilityCallback callback) {
  auto result = key_provider_->SignCapability(credential_id, zcap_json);
  if (!result) {
    std::move(callback).Run(nullptr);
    return;
  }
...
void SigningService::SignRaw(const std::string& credential_id,
                             const std::vector<uint8_t>& payload,
                             SignRawCallback callback) {
  auto signature = key_provider_->SignRaw(credential_id, payload);
  if (!signature) {
